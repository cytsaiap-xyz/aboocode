---
title: Server
description: Nutze den aboocode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `aboocode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein aboocode-Client nutzen kann.
...
Setze `ABOOCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `aboocode`, kann aber mit `ABOOCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `aboocode serve` und `aboocode web`.
...
Wenn du `aboocode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den aboocode-Server, um aboocode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `aboocode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `aboocode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Aboocode-[IDE](/docs/ide)-Plugins.
...
Der aboocode-Server stellt folgende APIs bereit.
