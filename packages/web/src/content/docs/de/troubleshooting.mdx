...
Wenn Aboocode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `aboocode --log-level DEBUG`.

---

## Speicher

aboocode speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
Aboocode Desktop startet im Hintergrund einen lokalen Aboocode-Server (`aboocode-cli`-Sidecar).
...
- Nur macOS: `Aboocode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit Aboocode hast:
