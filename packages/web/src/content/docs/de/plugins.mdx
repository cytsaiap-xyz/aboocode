---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Aboocode zu erweitern.
---

Mit Plugins können Sie Aboocode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Aboocode zu ändern.
...
- `.aboocode/plugins/` – Plugins auf Projektebene
- `~/.config/aboocode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/aboocode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/aboocode/aboocode.json`)
2. Projektkonfiguration (`aboocode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/aboocode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.aboocode/plugins/`)
...
Aboocode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Aboocode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Aboocode erweitern können.
...
:::note
Wenn Sie die Aboocode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Aboocode `.env`-Dateien liest:
...
Plugins können Aboocode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Aboocode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Aboocode zur Verfügung.
