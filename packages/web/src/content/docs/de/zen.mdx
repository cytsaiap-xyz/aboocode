---
title: Zen
description: Kuratierte Modellliste direkt von Aboocode.
---
...
Aboocode Zen ist eine vom Aboocode-Team getestete und verifizierte Modellliste.

:::note
Aboocode Zen befindet sich aktuell in der Beta.
:::

Zen funktioniert wie jeder andere Provider in Aboocode.
Du meldest dich bei Aboocode Zen an, holst dir deinen API-Key und nutzt ihn optional.
...
:::tip
Wir haben eine Auswahl aus Modellen und Providern getestet, die gut mit Aboocode funktionieren.
:::
...
Aboocode Zen ist ein AI-Gateway, das dir Zugriff auf genau diese Modelle gibt.

---

## Funktionsweise

Aboocode Zen funktioniert wie jeder andere Provider in Aboocode.

1. Melde dich bei **<a href={console}>Aboocode Zen</a>** an, hinterlege Zahlungsdaten und kopiere deinen API-Key.
2. Fuehre in der TUI `/connect` aus, waehle Aboocode Zen und fuege den API-Key ein.
...
Die [Model-ID](/docs/config/#models) in deiner Aboocode-Konfiguration hat das Format `aboocode/<model-id>`.
Fuer GPT 5.2 Codex verwendest du zum Beispiel `aboocode/gpt-5.2-codex`.
...
Wir haben Aboocode Zen entwickelt, um:
...
4. **Kein Lock-in** zu erzwingen, damit du Zen mit anderen Coding-Agents und Aboocode weiter mit anderen Providern nutzen kannst
