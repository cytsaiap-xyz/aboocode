...
Agent Skills erlauben Aboocode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Aboocode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Aboocode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.aboocode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/aboocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.aboocode/skills/git-release/SKILL.md` zum Beispiel so:
...
Aboocode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `aboocode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `aboocode.json`):
