---
title: Regeln
description: Lege eigene Anweisungen fuer aboocode fest.
---

Du kannst aboocode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in aboocode den Befehl `/init` aus.
...
So kann aboocode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht aboocode sie zu erweitern.
...
aboocode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/aboocode/AGENTS.md` speichern.
Diese gelten in allen aboocode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Aboocode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/aboocode/AGENTS.md` existiert)
...
Beim Start sucht aboocode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/aboocode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `aboocode.json` oder in `~/.config/aboocode/aboocode.json` angeben.
...
aboocode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit aboocode.json

Empfohlen ist das `instructions`-Feld in der `aboocode.json`:
...
Du kannst aboocode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `aboocode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
