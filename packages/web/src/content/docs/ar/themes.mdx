...
مع Aboocode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Aboocode سمتنا `aboocode`.
...
يأتي Aboocode مع عدة سمات مدمجة.
...
- يريدون أن يطابق Aboocode مظهر terminal لديهم
...
يدعم Aboocode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/aboocode/themes/*.json` أو `$XDG_CONFIG_HOME/aboocode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.aboocode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.aboocode/themes/*.json`
