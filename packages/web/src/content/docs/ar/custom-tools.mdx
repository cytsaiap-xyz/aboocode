...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في aboocode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في aboocode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.aboocode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/aboocode/tools/`.
