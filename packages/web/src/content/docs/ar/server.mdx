---
title: الخادم
description: تفاعل مع خادم aboocode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `aboocode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل aboocode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `aboocode.local` |
...
عيّن `ABOOCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `aboocode`، أو عيّن `ABOOCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `aboocode serve` و `aboocode web`.
...
عند تشغيل `aboocode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم aboocode للتفاعل مع aboocode برمجيا.
:::

تتيح هذه البنية لـ aboocode دعم عدة عملاء وتمكّنك من التفاعل مع aboocode برمجيا.

يمكنك تشغيل `aboocode serve` لبدء خادم مستقل. إذا كانت واجهة aboocode في terminal (TUI)
قيد التشغيل، فسيبدأ `aboocode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Aboocode لـ [IDE](/docs/ide).
...
يعرض خادم aboocode واجهات API التالية.
