---
title: Server
description: Interact with aboocode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `aboocode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an aboocode client can use.
...
Set `ABOOCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `aboocode`, or set `ABOOCODE_SERVER_USERNAME` to override it. This applies to both `aboocode serve` and `aboocode web`.
...
When you run `aboocode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the aboocode server to interact with aboocode programmatically.
:::

This architecture lets aboocode support multiple clients and allows you to interact with aboocode programmatically.

You can run `aboocode serve` to start a standalone server. If you have the
aboocode TUI running, `aboocode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Aboocode [IDE](/docs/ide) plugins.
...
The aboocode server exposes the following APIs.
