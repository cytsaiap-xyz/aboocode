---
title: Zen
description: Curated list of models provided by Aboocode.
---
...
Aboocode Zen is a list of tested and verified models provided by the Aboocode team.

:::note
Aboocode Zen is currently in beta.
:::

Zen works like any other provider in Aboocode. You login to Aboocode Zen and get
...
:::tip
We tested a select group of models and providers that work well with Aboocode.
:::
...
Aboocode Zen is an AI gateway that gives you access to these models.

---

## How it works

Aboocode Zen works like any other provider in Aboocode.

1. You sign in to **<a href={console}>Aboocode Zen</a>**, add your billing
   details, and copy your API key.
2. You run the `/connect` command in the TUI, select Aboocode Zen, and paste your API key.
...
The [model id](/docs/config/#models) in your Aboocode config
uses the format `aboocode/<model-id>`. For example, for GPT 5.2 Codex, you would
use `aboocode/gpt-5.2-codex` in your config.
...
- MiniMax M2.5 Free is available on Aboocode for a limited time. The team is using this time to collect feedback and improve the model.
- Big Pickle is a stealth model that's free on Aboocode for a limited time. The team is using this time to collect feedback and improve the model.
...
4. Have **no lock-in** by allowing you to use it with any other coding agent. And always let you use any other provider with Aboocode as well.
