...
Создайте `.aboocode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию aboocode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем aboocode [config](/docs/config):
...
- Глобальный: `~/.config/aboocode/commands/`
- Для каждого проекта: `.aboocode/commands/`
...
aboocode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
