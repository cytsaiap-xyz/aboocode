...
Aboocode has a list of keybinds that you can customize through the Aboocode config.
...
Aboocode uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The Aboocode desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `aboocode.json`.
