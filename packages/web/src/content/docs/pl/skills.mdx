...
Umiejętności agenta pozwalają aboocode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
aboocode przeszukuje te lokalizacje:

- Project config: `.aboocode/skills/<name>/SKILL.md`
- Global config: `~/.config/aboocode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu aboocode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.aboocode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/aboocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.aboocode/skills/git-release/SKILL.md` w ten sposób:
...
aboocode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `aboocode.json`:
...
**Dla agentów wbudowanych** (w `aboocode.json`):
