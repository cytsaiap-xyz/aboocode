...
W Aboocode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Aboocode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Aboocode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Aboocode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `aboocode.json`:
...
- Globalnie: `~/.config/aboocode/agents/`
- W projekcie: `.aboocode/agents/`
...
Jeśli nie zostanie skonfigurowana, Aboocode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Aboocode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Aboocode w `provider/model-id`. Na przykład, [Aboocode Zen](/docs/zen), użyj `aboocode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `aboocode models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/cytsaiap-xyz/aboocode).
:::
