---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w aboocode.
---

import config from "../../../../config.mjs"
export const console = config.console

aboocode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji aboocode.
...
w `~/.local/share/aboocode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim aboocode
config.
...
Aboocode Zen to lista modeli dostarczonych przez zespół aboocode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z aboocode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Aboocode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz aboocode i przejdź do [opencode.ai/auth](https://opencode.ai/auth).
...
Działa jak każdy inny dostawca w aboocode i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z aboocode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania aboocode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `aboocode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w aboocode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod aboocode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji aboocode.
...
aboocode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować aboocode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`aboocode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Customize through `aboocode.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem aboocode:
...
Aby używać Google Vertex AI z aboocode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Set them while running aboocode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą aboocode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [aboocode-helicone-session](https://github.com/H2Shami/aboocode-helicone-session), aby automatycznie rejestrować każdą konwersację aboocode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja aboocode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować aboocode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować aboocode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować aboocode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem aboocode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/aboocode).
:::
...
Aby korzystać z Ollama Cloud z aboocode:
...
3. Skopiuj klucz API do użycia w aboocode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w aboocode musisz pobrać informacje o modelu lokalnie:
...
Aboocode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół aboocode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Aboocode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Aboocode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API aboocode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji aboocode.
...
5. Możesz je także dostosować za pomocą konfiguracji aboocode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z aboocode:
...
Możesz także dostosować modele za pomocą konfiguracji aboocode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z aboocode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `aboocode.json` w katalogu projektu:
...
Pola `limit` pozwalają aboocode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `aboocode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację aboocode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji aboocode.
