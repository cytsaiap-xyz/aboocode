...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Aboocode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Aboocode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `ABOOCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `ABOOCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Aboocode lub gdy zmienna środowiskowa `ABOOCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Aboocode:
