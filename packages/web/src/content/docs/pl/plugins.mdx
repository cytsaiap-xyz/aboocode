---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć aboocode.
---

Wtyczki umożliwiające wyłączenie aboocode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania aboocode.
...
- `.aboocode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/aboocode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/aboocode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/aboocode/aboocode.json`)
2. Konfiguracja projektu (`aboocode.json`)
3. Globalny katalog wtyczek (`~/.config/aboocode/plugins/`)
4. Katalog wtyczek projektu (`.aboocode/plugins/`)
...
aboocode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z aboocode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do aboocode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej aboocode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj aboocode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do aboocode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod aboocode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
