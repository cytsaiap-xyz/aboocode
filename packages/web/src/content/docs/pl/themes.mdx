...
Dzięki aboocode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie aboocode używa naszego własnego motywu `aboocode`.
...
aboocode ma kilka wbudowanych motywów.
...
- Want aboocode to match their terminal's appearance
...
aboocode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/aboocode/themes/*.json` lub `$XDG_CONFIG_HOME/aboocode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.aboocode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.aboocode/themes/*.json`
