---
title: Serwer
description: Interakcja z serwerem aboocode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `aboocode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient aboocode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `aboocode.local` |
...
Ustaw `ABOOCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `aboocode` lub ustaw `ABOOCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `aboocode serve`, jak i `aboocode web`.
...
Kiedy uruchomisz `aboocode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera aboocode do programowej interakcji z aboocode.
:::

Ta architektura umożliwia obsługę wielu klientów przez aboocode i programową interakcję z kodem otwartym.

Możesz uruchomić `aboocode serve`, aby uruchomić samodzielny serwer. Jeśli masz
aboocode TUI działa, `aboocode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki aboocode [IDE](/docs/ide).
...
Serwer aboocode udostępnia następujące interfejsy API.
