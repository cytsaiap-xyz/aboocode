...
Narzędzia stosowane do funkcji, z których LLM może korzystać podczas rozmów. Współpracują z [wbudowanymi narzędziami] (./tools) aboocode, wtyczka jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.aboocode/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/aboocode/tools/`.
