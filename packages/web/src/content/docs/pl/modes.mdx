...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji aboocode.
...
te lub skonfiguruj własne za pomocą konfiguracji aboocode.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.aboocode/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `aboocode.json`:
...
- Globalnie: `~/.config/aboocode/modes/`
...
Jeśli nie ma wpływu na temperaturę, aboocode stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna aboocode, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.aboocode/modes/` dla trybów zapisanych dla projektu lub `~/.config/aboocode/modes/` dla trybów globalnych:
