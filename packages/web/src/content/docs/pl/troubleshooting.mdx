...
Aby debugować problemy z aboocode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\aboocode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `aboocode --log-level DEBUG`.

---

## Przechowywanie danych

aboocode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/aboocode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\aboocode`
...
aboocode Desktop uruchamia w tle lokalny serwer aboocode (sidecar `aboocode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Aboocode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/aboocode/aboocode.jsonc` (lub `~/.config/aboocode/aboocode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/aboocode/aboocode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\aboocode\aboocode.jsonc`
...
aboocode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/aboocode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\aboocode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby aboocode mógł ją odbudować.

1. Całkowicie zamknij aboocode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/aboocode`
- **Linux**: usuń `~/.cache/aboocode` (lub uruchom `rm -rf ~/.cache/aboocode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\aboocode`

3. Uruchom ponownie aboocode Desktop.

---

### Napraw problemy z połączeniem z serwerem

aboocode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `aboocode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `ABOOCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `ABOOCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows aboocode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami aboocode.

---

### Brak powiadomień

aboocode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla aboocode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie aboocode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji aboocode Desktop):

- `aboocode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `aboocode.global.dat` i `aboocode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z aboocode:
...
3. Upewnij się, że masz najnowszą wersję z `aboocode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `aboocode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\aboocode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. aboocode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\aboocode`

2. Uruchom ponownie kod aboocode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to aboocode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
aboocode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
