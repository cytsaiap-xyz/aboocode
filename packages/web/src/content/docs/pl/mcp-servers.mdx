...
Możesz dodać dodatkowe informacje do aboocode za pomocą _Model Context Protocol_, w skrócie MCP. aboocode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [aboocode Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/aboocode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
aboocode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, aboocode:
...
Jeśli serwer wymaga uwierzytelnienia, aboocode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `aboocode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji aboocode bezpieczne przechowa tokeny w `~/.local/share/aboocode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w aboocode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie aboocode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć aboocode z kontem Sentry.
