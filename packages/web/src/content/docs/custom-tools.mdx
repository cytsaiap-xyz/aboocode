---
title: Custom Tools
description: Create tools the LLM can call in aboocode.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside aboocode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.aboocode/tools/` directory of your project.
- Or globally, by placing them in `~/.config/aboocode/tools/`.
...
```ts title=".aboocode/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
