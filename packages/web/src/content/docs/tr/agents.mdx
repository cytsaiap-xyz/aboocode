...
aboocode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
aboocode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
aboocode, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

aboocode iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `aboocode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse aboocode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global aboocode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
aboocode hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [Aboocode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `aboocode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `aboocode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/cytsaiap-xyz/aboocode).
:::
