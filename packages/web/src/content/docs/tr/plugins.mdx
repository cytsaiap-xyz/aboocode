---
title: Eklentiler
description: aboocode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek aboocode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya aboocode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.aboocode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/aboocode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/aboocode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/aboocode/aboocode.json`)
2. Project config (`aboocode.json`)
3. Global eklenti dizini (`~/.config/aboocode/plugins/`)
4. Proje eklenti dizini (`.aboocode/plugins/`)
...
aboocode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için aboocode'lu bir SDK istemcisi.
...
aboocode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
aboocode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
aboocode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca aboocode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, aboocode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
