...
Ajan becerileri, aboocode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
aboocode su konumlari tarar:

- Proje konfigurasyonu: `.aboocode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/aboocode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda aboocode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.aboocode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/aboocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.aboocode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
aboocode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`aboocode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`aboocode.json` icinde):
