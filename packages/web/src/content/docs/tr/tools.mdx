...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. aboocode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
aboocode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `ABOOCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `ABOOCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca aboocode provider kullanılırken veya `ABOOCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

aboocode başlatırken etkinleştirmek için:
