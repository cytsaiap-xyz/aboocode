---
title: Sunucu
description: aboocode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`aboocode serve` komutu, aboocode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `aboocode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `ABOOCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `aboocode` değeridir; değiştirmek isterseniz `ABOOCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `aboocode serve` hem de `aboocode web` için geçerlidir.
...
`aboocode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
aboocode ile programatik etkileşim için aboocode sunucusunu kullanın.
:::

Bu mimari, aboocode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`aboocode serve` ile bağımsız bir sunucu başlatabilirsiniz. aboocode TUI açıksa, `aboocode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum aboocode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
aboocode sunucusu aşağıdaki API'leri sunar.
