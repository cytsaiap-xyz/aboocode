---
title: Sağlayıcılar
description: aboocode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

aboocode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı aboocode yapılandırmanızda yapılandırın.
...
`~/.local/share/aboocode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları aboocode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Aboocode Zen, aboocode ekibi tarafından sağlanan modellerin bir listesidir.
aboocode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Aboocode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, aboocode'u seçin ve [opencode.ai/auth](https://opencode.ai/auth)'ye gidin.
...
aboocode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u aboocode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   aboocode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `aboocode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi aboocode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   aboocode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Aboocode yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in aboocode auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
aboocode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. aboocode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`aboocode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`aboocode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi aboocode'la kullanmak için:
...
Google Vertex AI'yi aboocode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları aboocode çalıştırılırken ayarlayın.
...
Helicone'un aboocode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her aboocode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [aboocode-helicone-session](https://github.com/H2Shami/aboocode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her aboocode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
aboocode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla aboocode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla aboocode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini aboocode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/aboocode)'a bakın.
:::
...
Ollama Cloud'u aboocode ile kullanmak için:
...
3. aboocode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: aboocode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Aboocode Zen, aboocode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Aboocode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Aboocode Zen**'i arayın.

   ```txt
   /connect
   ```

3. aboocode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   aboocode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca aboocode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Aboocode ile kullanmak için:
...
Modelleri aboocode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı aboocode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `aboocode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları aboocode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `aboocode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için aboocode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, aboocode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
