...
description: aboocode, LSP sunucularınızla bütünleşir.
---

aboocode, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

aboocode, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`ABOOCODE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
Aboocode bir dosyayı açtığında:
...
LSP sunucularını, aboocode yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
