...
aboocode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak aboocode kendi `aboocode` temasını kullanır.
...
aboocode birden fazla yerleşik temayla gelir.
...
- aboocode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
aboocode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/aboocode/themes/*.json` veya `$XDG_CONFIG_HOME/aboocode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.aboocode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.aboocode/themes/*.json`
