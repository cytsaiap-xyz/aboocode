...
:::caution
Modlar artık aboocode yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

aboocode'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya aboocode yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
aboocode iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.aboocode/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`aboocode.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, aboocode modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global aboocode yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.aboocode/modes/` veya genel modlar için `~/.config/aboocode/modes/`'de mod dosyaları oluşturun:
