...
_Model Bağlam Protokolü_ veya MCP kullanarak aboocode'a harici araçlar ekleyebilirsiniz. aboocode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [aboocode Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/aboocode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
aboocode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde aboocode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa aboocode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `aboocode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından aboocode, belirteçleri `~/.local/share/aboocode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra aboocode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi aboocode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve aboocode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
