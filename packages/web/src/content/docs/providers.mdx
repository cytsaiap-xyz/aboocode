---
title: Providers
description: Using any LLM provider in Aboocode.
---

import config from "../../../config.mjs"
export const console = config.console

Aboocode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Aboocode config.
...
You can customize the providers through the `provider` section in your Aboocode
config.
...
Aboocode Zen is a list of models provided by the Aboocode team that have been
tested and verified to work well with Aboocode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Aboocode Zen.
:::

1. Run the `/connect` command in the TUI, select `Aboocode Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in Aboocode and is completely optional to use.

---

## Aboocode Go

Aboocode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Aboocode team that have been
tested and verified to work well with Aboocode.

1. Run the `/connect` command in the TUI, select `Aboocode Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with Aboocode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running aboocode:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `aboocode.json`:
...
:::info
Using your Claude Pro/Max subscription in Aboocode is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for aboocode to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your aboocode config.
...
Aboocode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Aboocode
to only use your own GitLab-hosted instance, add the following to your
`aboocode.json` file. It is also recommended to disable session sharing.
...
To use your GitHub Copilot subscription with aboocode:
...
To use Google Vertex AI with Aboocode:
...
In the event you see a feature or model from Helicone that isn't configured automatically through aboocode, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [aboocode-helicone-session](https://github.com/H2Shami/aboocode-helicone-session) plugin to automatically log each Aboocode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Aboocode conversation listed as a separate session.
...
You can configure aboocode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure aboocode to use local models through LM Studio.
...
You can configure aboocode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Aboocode. See the [Ollama integration docs](https://docs.ollama.com/integrations/aboocode) for details.
:::
...
To use Ollama Cloud with Aboocode:
...
3. Copy the API key for use in Aboocode.
...
6. **Important**: Before using cloud models in Aboocode, you must pull the model information locally:
...
Aboocode Zen is a list of tested and verified models provided by the Aboocode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Aboocode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Aboocode Zen**.

   ```txt
   /connect
   ```

3. Enter your Aboocode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your aboocode config.
...
5. You can also customize them through your aboocode config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Aboocode:
...
You can also customize models through your aboocode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with aboocode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `aboocode.json` file in your project directory:
...
The `limit` fields allow Aboocode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `aboocode auth list` to see if the credentials
...
2. For custom providers, check the aboocode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your aboocode config.
