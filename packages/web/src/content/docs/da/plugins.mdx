---
title: Plugins
description: Skriv dine egne plugins for at udvide Aboocode.
---

Plugins giver dig mulighed for at udvide Aboocode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Aboocodes standardadfærd.
...
- `.aboocode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/aboocode/node_modules/`.
...
1. Global konfiguration (`~/.config/aboocode/aboocode.json`)
2. Projektkonfiguration (`aboocode.json`)
3. Global plugin-mappe (`~/.config/aboocode/plugins/`)
4. Projekt plugin bibliotek (`.aboocode/plugins/`)
...
Aboocode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En aboocode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide aboocode.
...
:::note
Hvis du bruger Aboocode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå aboocode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til aboocode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som aboocode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for aboocode sammen med indbyggede værktøjer.
