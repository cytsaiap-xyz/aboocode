...
For at fejlfinde problemer med Aboocode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\aboocode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `aboocode --log-level DEBUG`.

---

## Lagring

aboocode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/aboocode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\aboocode`
...
Aboocode Desktop kører en lokal Aboocode-server (`aboocode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Aboocode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/aboocode/aboocode.jsonc` (eller `~/.config/aboocode/aboocode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/aboocode/aboocode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\aboocode\aboocode.jsonc`
...
Aboocode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/aboocode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\aboocode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Aboocode kan genopbygge den.

1. Afslut Aboocode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/aboocode`
- **Linux**: slet `~/.cache/aboocode` (eller kør `rm -rf ~/.cache/aboocode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\aboocode`

3. Start Aboocode Desktop på ny.

---

### Løs problemer med serverforbindelse

Aboocode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `aboocode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `ABOOCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `ABOOCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Aboocode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Aboocode.

---

### Meddelelser vises ikke

Aboocode Desktop viser kun systemvarsler når:

- varsler er aktiveret for Aboocode i dine OS-indstillinger, og
...
1. Afslut Aboocode Desktop.
2. Find og slet disse filer (de findes i Aboocode Desktop-appens datamappe):

- `aboocode.settings.dat` (skrivebordsstandardserver URL)
- `aboocode.global.dat` og `aboocode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Aboocode:
...
For at finde ud af hvilke modeller du har adgang til, kør `aboocode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\aboocode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. aboocode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\aboocode`

2. Start aboocode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge aboocode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
aboocode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
