...
Agentferdigheter lar Aboocode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Aboocode søker etter disse stedene:

- Prosjektkonfigurasjon: `.aboocode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/aboocode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Aboocode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.aboocode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/aboocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.aboocode/skills/git-release/SKILL.md` slik:
...
Aboocode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `aboocode.json`:
...
**For innebygde agenter** (i `aboocode.json`):
