...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i aboocode-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i aboocode giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem aboocode-konfigurationen.
...
aboocode leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.aboocode/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `aboocode.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger aboocode modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale aboocode-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.aboocode/modes/` for projektspecifikke tilstande eller `~/.config/aboocode/modes/` for globale tilstande:
