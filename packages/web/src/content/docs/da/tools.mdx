...
Her er alle de innebygde verktøyene som er tilgængelige i Aboocode.
...
:::note
Dette verktøyet er bare tilgængelig når `ABOOCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `ABOOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Aboocode-leverandøren eller når miljøvariabelen `ABOOCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
