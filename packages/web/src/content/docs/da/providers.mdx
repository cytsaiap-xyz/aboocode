---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Aboocode.
---

import config from "../../../../config.mjs"
export const console = config.console

Aboocode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Aboocode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/aboocode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Aboocode-konfigurationen.
...
Aboocode Zen er en liste over modeller leveret af Aboocode-teamet, som er blevet testet og verificeret til at fungere godt med Aboocode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Aboocode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg aboocode og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i Aboocode og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med Aboocode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører aboocode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `aboocode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Aboocode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at aboocode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem aboocode-konfigurationen.
...
gemme tokenet i aboocode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Aboocode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Aboocode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`aboocode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Aboocode:
...
Sådan bruger du Google Vertex AI med Aboocode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører aboocode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem aboocode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [aboocode-helicone-session](https://github.com/H2Shami/aboocode-helicone-session) plugin for automatisk at logge hver Aboocode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Aboocode-samtale opført som en separat session.
...
Du kan konfigurere aboocode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere aboocode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere aboocode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Aboocode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/aboocode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Aboocode:
...
3. Kopier API-nøglen til brug i Aboocode.
...
6. **Vigtigt**: Før du bruger skymodeller i Aboocode, skal du hente modelinformationen lokalt:
...
Aboocode Zen er en liste over testede og verificerede modeller leveret af Aboocode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Aboocode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Aboocode Zen**.

   ```txt
   /connect
   ```

3. Indtast Aboocode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem aboocode-konfigurationen.
...
5. Du kan også tilpasse dem gennem aboocode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Aboocode:
...
Du kan også tilpasse modeller gennem aboocode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med aboocode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `aboocode.json`-filen i projektkataloget:
...
`limit`-felterne lader Aboocode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `aboocode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek aboocode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i aboocode-konfigurationen.
