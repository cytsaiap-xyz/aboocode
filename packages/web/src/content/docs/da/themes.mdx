...
Med Aboocode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Aboocode vårt eget `aboocode`-tema.
...
Aboocode kommer med flere innebygde temaer.
...
- Vil at Aboocode skal matche terminalens utseende
...
Aboocode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/aboocode/themes/*.json` eller `$XDG_CONFIG_HOME/aboocode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.aboocode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.aboocode/themes/*.json`
