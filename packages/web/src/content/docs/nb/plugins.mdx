...
description: Skriv dine egne programtillegg for å utvide aboocode.
---

Plugins lar deg utvide Aboocode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Aboocode.
...
- `.aboocode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/aboocode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/aboocode/aboocode.json`)
2. Prosjektkonfigurasjon (`aboocode.json`)
3. Global plugin-katalog (`~/.config/aboocode/plugins/`)
4. Prosjektpluginkatalog (`.aboocode/plugins/`)
...
Aboocode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Aboocode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Aboocode.
...
:::note
Hvis du bruker Aboocode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Aboocode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Aboocode:
...
`tool`-hjelperen lager et tilpasset verktøy som Aboocode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Aboocode sammen med innebygde verktøy.
