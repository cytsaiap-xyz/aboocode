...
Det er to typer agenter i Aboocode; primære agenter og underagenter.
...
Aboocode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Aboocode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Aboocode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `aboocode.json`:
...
- Globalt: `~/.config/aboocode/agents/`
- Per prosjekt: `.aboocode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Aboocode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Aboocode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Aboocode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Aboocode Zen](/docs/zen), vil du bruke `aboocode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `aboocode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/cytsaiap-xyz/aboocode).
:::
