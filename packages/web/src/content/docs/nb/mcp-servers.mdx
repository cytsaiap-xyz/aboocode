...
Du kan legge til eksterne verktøy i Aboocode ved å bruke _Model Context Protocol_ eller MCP. Aboocode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Aboocode Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="aboocode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/aboocode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Aboocode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Aboocode:
...
Hvis serveren krever autentisering, vil Aboocode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `aboocode mcp auth <server-name>`.
...
```bash
aboocode mcp auth my-oauth-server
```
...
```bash
aboocode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Aboocode lagre tokens sikkert i `~/.local/share/aboocode/mcp-auth.json`.
...
aboocode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Aboocode, sammen med innebygde verktøy. Så du kan administrere dem gjennom Aboocode-konfigurasjonen som alle andre verktøy.
...
```json title="aboocode.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
aboocode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Aboocode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Aboocode-samtale oppført som en separat økt.
