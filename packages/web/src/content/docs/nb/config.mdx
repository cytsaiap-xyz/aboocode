---
title: Konfigurasjon
description: Ved å bruke aboocode JSON-konfigurasjonen.
---

Du kan konfigurere Aboocode ved å bruke en JSON konfigurasjonsfil.

---

## Format

Aboocode støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "aboocode"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/aboocode`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/aboocode/aboocode.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`ABOOCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`aboocode.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.aboocode` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`ABOOCODE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.aboocode` og `~/.config/aboocode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/aboocode`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Aboocode-konfigurasjon i `~/.config/aboocode/aboocode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `aboocode.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Aboocode starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `ABOOCODE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `ABOOCODE_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.aboocode` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.aboocode`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan konfigurere serverinnstillinger for kommandoene `aboocode serve` og `aboocode web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Aboocode-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `aboocode.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Aboocode-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Aboocode å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan konfigurere temaet du vil bruke i Aboocode-konfigurasjonen gjennom alternativet `theme`.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/aboocode/agents/` eller `.aboocode/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Aboocode falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`aboocode run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/aboocode/commands/` eller `.aboocode/commands/`. [Les mer her](/docs/commands).
...
Aboocode vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Aboocode alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Aboocode med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.aboocode/plugins/` eller `~/.config/aboocode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Aboocode til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
