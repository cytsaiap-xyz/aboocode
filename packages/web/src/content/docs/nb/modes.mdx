...
:::caution
Moduser er nå konfigurert gjennom alternativet `agent` i Aboocode-konfigurasjonen. De
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i Aboocode lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom Aboocode-konfigurasjonen.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.aboocode/plans/*.md` for å detaljere selve planen
...
Hvis ingen temperatur er spesifisert, bruker Aboocode modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
