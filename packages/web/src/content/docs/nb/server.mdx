---
title: Tjener
description: Samhandle med Aboocode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `aboocode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Aboocode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `aboocode.local` |
...
Sett `ABOOCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `aboocode` som standard, eller sett `ABOOCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `aboocode serve` og `aboocode web`.
...
Når du kjører `aboocode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Aboocode-serveren til å samhandle med Aboocode programmatisk.
:::

Denne arkitekturen lar Aboocode støtte flere klienter og lar deg samhandle med Aboocode programmatisk.

Du kan kjøre `aboocode serve` for å starte en frittstående server. Hvis du har
Aboocode TUI kjører, vil `aboocode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Aboocode [IDE](/docs/ide) plugins.
...
aboocode-serveren viser følgende APIer.
