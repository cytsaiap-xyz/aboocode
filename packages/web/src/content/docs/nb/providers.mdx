---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Aboocode.
---

import config from "../../../../config.mjs"
export const console = config.console

Aboocode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Aboocode-konfigurasjonen.
...
i `~/.local/share/aboocode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Aboocode
konfig.
...
Aboocode Zen er en liste over modeller levert av Aboocode-teamet som har vært
testet og verifisert for å fungere godt med Aboocode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Aboocode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg aboocode og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre leverandører i Aboocode og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med Aboocode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Aboocode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `aboocode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i aboocode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Aboocode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Aboocode-konfigurasjonen.
...
for å lagre token i Aboocode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Aboocode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Aboocode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`aboocode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med Aboocode:
...
Slik bruker du Google Vertex AI med Aboocode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Aboocode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Aboocode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [aboocode-helicone-session](https://github.com/H2Shami/aboocode-helicone-session) plugin for automatisk å logge hver Aboocode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Aboocode-samtale oppført som en separat økt.
...
Du kan konfigurere Aboocode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Aboocode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Aboocode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Aboocode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/aboocode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Aboocode:
...
3. Kopier API-nøkkelen for bruk i Aboocode.
...
6. **Viktig**: Før du bruker skymodeller i Aboocode, må du hente modellinformasjonen lokalt:
...
Aboocode Zen er en liste over testede og verifiserte modeller levert av Aboocode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Aboocode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Aboocode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Aboocode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Aboocode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Aboocode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Aboocode:
...
Du kan også tilpasse modeller gjennom Aboocode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Aboocode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `aboocode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Aboocode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `aboocode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Aboocode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Aboocode-konfigurasjonen.
