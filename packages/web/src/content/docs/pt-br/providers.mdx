---
title: Provedores
description: Usando qualquer provedor LLM no aboocode.
---

import config from "../../../../config.mjs"
export const console = config.console

aboocode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do aboocode.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/aboocode/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do aboocode.
...
Aboocode Zen é uma lista de modelos fornecidos pela equipe do aboocode que foram testados e verificados para funcionar bem com o aboocode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Aboocode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione aboocode e acesse [opencode.ai/auth](https://opencode.ai/auth).
...
Funciona como qualquer outro provedor no aboocode e é completamente opcional.
...
Para usar o Amazon Bedrock com o aboocode:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   Defina uma dessas variáveis de ambiente ao executar o aboocode:
   ...
   Para configuração específica do projeto ou persistente, use `aboocode.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no aboocode não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o aboocode funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do aboocode.
...
armazenar o token no armazenamento de autenticação do aboocode.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
aboocode usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o aboocode
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `aboocode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `aboocode.json`:
...
Para usar sua assinatura do GitHub Copilot com o aboocode:
...
Para usar o Google Vertex AI com o aboocode:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o aboocode.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do aboocode, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [aboocode-helicone-session](https://github.com/H2Shami/aboocode-helicone-session) para registrar automaticamente cada conversa do aboocode como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do aboocode listada como uma sessão separada.
...
Você pode configurar o aboocode para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o aboocode para usar modelos locais através do LM Studio.
...
Você pode configurar o aboocode para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o aboocode. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/aboocode) para detalhes.
:::
...
Para usar o Ollama Cloud com o aboocode:
...
3. Copie a chave da API para uso no aboocode.
...
6. **Importante**: Antes de usar modelos em nuvem no aboocode, você deve puxar as informações do modelo localmente:
...
Aboocode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do aboocode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Aboocode Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Aboocode Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do aboocode.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do aboocode.
...
5. Você também pode personalizá-los através da sua configuração do aboocode. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o aboocode:
...
Você também pode personalizar modelos através da sua configuração do aboocode. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o aboocode. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
1. Execute o comando `/connect` e role para baixo até **Outro**.
   ...
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no aboocode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `aboocode.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o aboocode entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `aboocode auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do aboocode e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do aboocode.
