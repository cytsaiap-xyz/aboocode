...
As habilidades do agente permitem que o aboocode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O aboocode pesquisa nesses locais:

- Configuração do projeto: `.aboocode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/aboocode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o aboocode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.aboocode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/aboocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.aboocode/skills/git-release/SKILL.md` assim:
...
O aboocode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `aboocode.json`:
...
**Para agentes embutidos** (em `aboocode.json`):
