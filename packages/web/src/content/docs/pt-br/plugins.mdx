---
title: Plugins
description: Escreva seus próprios plugins para estender o aboocode.
---

Plugins permitem que você estenda o aboocode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do aboocode.
...
- `.aboocode/plugins/` - Plugins em nível de projeto
- `~/.config/aboocode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/aboocode/node_modules/`.
...
1. Configuração global (`~/.config/aboocode/aboocode.json`)
2. Configuração do projeto (`aboocode.json`)
3. Diretório de plugins global (`~/.config/aboocode/plugins/`)
4. Diretório de plugins do projeto (`.aboocode/plugins/`)
...
O aboocode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do aboocode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o aboocode.
...
:::note
Se você estiver usando o aplicativo desktop aboocode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o aboocode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao aboocode:
...
O helper `tool` cria uma ferramenta personalizada que o aboocode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o aboocode junto com as ferramentas integradas.
