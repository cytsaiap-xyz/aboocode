---
title: Servidor
description: Interaja com o servidor aboocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `aboocode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente aboocode pode usar.
...
```bash
aboocode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `aboocode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
aboocode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `ABOOCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `aboocode`, ou defina `ABOOCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `aboocode serve` quanto ao `aboocode web`.

```bash
ABOOCODE_SERVER_PASSWORD=your-password aboocode serve
```
...
Quando você executa `aboocode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor aboocode para interagir com o aboocode programaticamente.
:::

Essa arquitetura permite que o aboocode suporte múltiplos clientes e permite que você interaja com o aboocode programaticamente.

Você pode executar `aboocode serve` para iniciar um servidor autônomo. Se você tiver o TUI do aboocode em execução, `aboocode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do aboocode [IDE](/docs/ide).
...
O servidor aboocode expõe as seguintes APIs.
