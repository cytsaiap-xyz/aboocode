...
Com o aboocode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o aboocode usa nosso próprio tema `aboocode`.
...
O aboocode vem com vários temas embutidos.
...
- Querem que o aboocode corresponda à aparência do seu terminal
...
O aboocode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/aboocode/themes/*.json` ou `$XDG_CONFIG_HOME/aboocode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.aboocode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.aboocode/themes/*.json`
