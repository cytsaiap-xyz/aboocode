...
Le skill degli agenti permettono a Aboocode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Aboocode cerca in queste posizioni:

- Config di progetto: `.aboocode/skills/<name>/SKILL.md`
- Config globale: `~/.config/aboocode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Aboocode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.aboocode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/aboocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.aboocode/skills/git-release/SKILL.md` cosi':
...
Aboocode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `aboocode.json`:
...
**Per agenti integrati** (in `aboocode.json`):
