---
title: Server
description: Interagisci con il server aboocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `aboocode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client aboocode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `aboocode.local` |
...
Imposta `ABOOCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `aboocode`, oppure imposta `ABOOCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `aboocode serve` sia per `aboocode web`.
...
Quando esegui `aboocode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server aboocode per interagire con aboocode in modo programmatico.
:::

Questa architettura permette a aboocode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `aboocode serve` per avviare un server standalone. Se la TUI di aboocode e gia in esecuzione, `aboocode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Aboocode.
...
Il server aboocode espone le seguenti API.
