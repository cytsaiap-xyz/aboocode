---
title: Plugin
description: Scrivi plugin per estendere Aboocode.
---

I plugin ti permettono di estendere Aboocode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di Aboocode.
...
- `.aboocode/plugins/` - plugin a livello progetto
- `~/.config/aboocode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/aboocode/node_modules/`.
...
1. Config globale (`~/.config/aboocode/aboocode.json`)
2. Config di progetto (`aboocode.json`)
3. Directory plugin globale (`~/.config/aboocode/plugins/`)
4. Directory plugin di progetto (`.aboocode/plugins/`)
...
Aboocode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di aboocode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere aboocode.
...
:::note
Se usi l'app desktop di Aboocode, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a aboocode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a aboocode:
...
L'helper `tool` crea uno strumento personalizzato che aboocode puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in aboocode insieme agli strumenti integrati.
