...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di aboocode. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in aboocode ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di aboocode.
...
aboocode include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.aboocode/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `aboocode.json`:
...
- Globale: `~/.config/aboocode/modes/`
- Progetto: `.aboocode/modes/`
...
Se non viene specificata alcuna temperatura, aboocode usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di aboocode sia per quella specifica del progetto.
...
Crea file di modalita in `.aboocode/modes/` per modalita specifiche del progetto oppure in `~/.config/aboocode/modes/` per modalita globali:
