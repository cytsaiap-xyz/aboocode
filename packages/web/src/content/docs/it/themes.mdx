...
Con Aboocode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Aboocode usa il tema `aboocode`.
...
Aboocode include diversi temi integrati.
...
- Vuole che Aboocode corrisponda all'aspetto del terminale
...
Aboocode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/aboocode/themes/*.json` o `$XDG_CONFIG_HOME/aboocode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.aboocode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.aboocode/themes/*.json`
