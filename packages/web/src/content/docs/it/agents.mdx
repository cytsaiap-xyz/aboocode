...
In Aboocode esistono due tipi di agenti: agenti primari e subagenti.
...
Aboocode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Aboocode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Aboocode include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `aboocode.json`:
...
- Globale: `~/.config/aboocode/agents/`
- Per progetto: `.aboocode/agents/`
...
Se non specifichi una temperature, Aboocode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Aboocode sia per la config specifica del progetto.
...
L'ID modello nella configurazione Aboocode usa il formato `provider/model-id`. Per esempio, se stai usando [Aboocode Zen](/docs/zen), useresti `aboocode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `aboocode models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/cytsaiap-xyz/aboocode).
:::
