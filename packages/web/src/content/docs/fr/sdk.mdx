---
title: SDK
description: Client JS de type sécurisé pour le serveur aboocode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK aboocode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler aboocode par programme.
...
Créez une instance de aboocode :

```javascript
import { createAboocode } from "@opencode-ai/sdk"

const { client } = await createAboocode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `aboocode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${aboocode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de aboocode, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createAboocodeClient } from "@opencode-ai/sdk"
...
const textResults = await client.find.text({
  query: { pattern: "function.*aboocode" },
})
