...
description: Créez des outils que le LLM peut appeler dans aboocode.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de aboocode comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.aboocode/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/aboocode/tools/`.
...
```ts title=".aboocode/tools/database.ts" {1}
...
```ts title=".aboocode/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".aboocode/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".aboocode/tools/add.py"
import sys
...
```ts title=".aboocode/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".aboocode/tools/add.py")
