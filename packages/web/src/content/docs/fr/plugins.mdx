---
title: Plugins
description: Écrivez vos propres plugins pour étendre Aboocode.
---

Les plugins vous permettent d'étendre Aboocode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Aboocode.
...
- `.aboocode/plugins/` - Plugins au niveau du projet
- `~/.config/aboocode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/aboocode/node_modules/`.
...
1. Configuration globale (`~/.config/aboocode/aboocode.json`)
2. Configuration du projet (`aboocode.json`)
3. Répertoire global des plugins (`~/.config/aboocode/plugins/`)
4. Répertoire des plugins du projet (`.aboocode/plugins/`)
...
Aboocode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK aboocode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre aboocode.
...
:::note
Si vous utilisez l'application de bureau Aboocode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez aboocode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à aboocode :
...
L'assistant `tool` crée un outil personnalisé que aboocode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour aboocode aux côtés des outils intégrés.
