...
Avec Aboocode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Aboocode utilise notre propre thème `aboocode`.
...
Aboocode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Aboocode corresponde à l'apparence de leur terminal
...
Aboocode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/aboocode/themes/*.json` ou `$XDG_CONFIG_HOME/aboocode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.aboocode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.aboocode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/aboocode/themes
vim ~/.config/aboocode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .aboocode/themes
vim .aboocode/themes/my-theme.json
```
