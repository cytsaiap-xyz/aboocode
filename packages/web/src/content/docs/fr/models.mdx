...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Aboocode.
...
Voici plusieurs modèles qui fonctionnent bien avec Aboocode, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Aboocode.

```json title="aboocode.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Aboocode Zen](/docs/zen), vous utiliserez `aboocode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="aboocode.jsonc" {7-12,19-24}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="aboocode.jsonc" {6-21}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "aboocode": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Aboocode est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Aboocode est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="aboocode.jsonc" {7-18}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque Aboocode démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Aboocode.
