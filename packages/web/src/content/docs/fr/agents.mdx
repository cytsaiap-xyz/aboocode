...
Il existe deux types d'agents dans Aboocode ; agents primaires et sous-agents.
...
Aboocode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Aboocode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Aboocode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `aboocode.json` :
...
- Global : `~/.config/aboocode/agents/`
- Par projet : `.aboocode/agents/`

```markdown title="~/.config/aboocode/agents/review.md"
---
...
Si aucune température n'est spécifiée, Aboocode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Aboocode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Aboocode utilise le format `provider/model-id`. Par exemple, si vous utilisez [Aboocode Zen](/docs/zen), vous utiliserez `aboocode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="aboocode.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="aboocode.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `aboocode models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/cytsaiap-xyz/aboocode).
:::
...
```markdown title="~/.config/aboocode/agents/docs-writer.md"
---
...
```markdown title="~/.config/aboocode/agents/security-auditor.md"
---
