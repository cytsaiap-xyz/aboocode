---
title: Configuration
description: Utilisation de la configuration JSON Aboocode.
---

Vous pouvez configurer Aboocode à l'aide d'un fichier de configuration JSON.

---

## Format

Aboocode prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="aboocode.jsonc"
{
...
Par exemple, si votre configuration globale définit `theme: "aboocode"` et `autoupdate: true` et que la configuration de votre projet définit `model: "anthropic/claude-sonnet-4-5"`, la configuration finale inclura les trois paramètres.
...
1. **Configuration distante** (à partir de `.well-known/aboocode`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/aboocode/aboocode.json`) - préférences utilisateur
3. **Configuration personnalisée** (`ABOOCODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`aboocode.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.aboocode`** - agents, commandes, plugins
6. **Configuration en ligne** (`ABOOCODE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.aboocode` et `~/.config/aboocode` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/aboocode`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/aboocode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale Aboocode dans `~/.config/aboocode/aboocode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les thèmes, les fournisseurs ou les raccourcis clavier.
...
Ajoutez `aboocode.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.

:::tip
Placez la configuration spécifique au projet à la racine de votre projet.
:::

Lorsque Aboocode démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `ABOOCODE_CONFIG`.

```bash
export ABOOCODE_CONFIG=/path/to/my/custom-config.json
aboocode run "Hello world"
```
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `ABOOCODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.aboocode`, et devrait suivre la même structure.

```bash
export ABOOCODE_CONFIG_DIR=/path/to/my/config-directory
aboocode run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.aboocode`, il **peut donc remplacer** leurs paramètres.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `aboocode serve` et `aboocode web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Aboocode.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `aboocode.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Aboocode via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Aboocode essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez configurer le thème que vous souhaitez utiliser dans votre configuration Aboocode via l'option `theme`.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/aboocode/agents/` ou `.aboocode/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Aboocode reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`aboocode run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/aboocode/commands/` ou `.aboocode/commands/`. [En savoir plus ici](/docs/commands).
...
Aboocode téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, aboocode **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Aboocode avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.aboocode/plugins/` ou `~/.config/aboocode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="aboocode.json"
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["aboocode-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre Aboocode à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
