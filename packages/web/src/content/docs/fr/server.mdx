---
title: Serveur
description: Interagissez avec le serveur aboocode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `aboocode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client aboocode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `aboocode.local` |
...
Définissez `ABOOCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `aboocode`, ou définissez `ABOOCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `aboocode serve` et à `aboocode web`.
...
Lorsque vous exécutez `aboocode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur aboocode pour interagir avec aboocode par programmation.
:::

Cette architecture permet à aboocode de prendre en charge plusieurs clients et vous permet d'interagir avec aboocode par programme.

Vous pouvez exécuter `aboocode serve` pour démarrer un serveur autonome. Si vous avez le
aboocode TUI en cours d'exécution, `aboocode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Aboocode [IDE](/docs/ide).
...
Le serveur aboocode expose les API suivantes.
