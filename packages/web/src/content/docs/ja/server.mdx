---
title: サーバー
description: HTTP 経由で Aboocode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`aboocode serve` コマンドは、Aboocode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `aboocode.local` |
...
HTTP 基本認証でサーバーを保護するには、`ABOOCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `aboocode` になるか、`ABOOCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`aboocode serve` と `aboocode web` の両方に当てはまります。
...
`aboocode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Aboocode サーバーを使用して、プログラムで Aboocode と対話します。
:::

`aboocode serve` を実行してスタンドアロンサーバーを起動できます。すでに Aboocode TUI を実行している場合でも、`aboocode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Aboocode [IDE](/docs/ide) プラグインによって使用されます。
