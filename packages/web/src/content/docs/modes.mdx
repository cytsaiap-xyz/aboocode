...
:::caution
Modes are now configured through the `agent` option in the aboocode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in aboocode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the aboocode config.
...
aboocode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.aboocode/plans/*.md` to detail the plan itself
...
Configure modes in your `aboocode.json` config file:
...
both the global aboocode config and the project specific config.
...
Create mode files in `.aboocode/modes/` for project-specific modes or `~/.config/aboocode/modes/` for global modes:
