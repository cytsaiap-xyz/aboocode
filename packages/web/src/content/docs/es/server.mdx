---
title: Servidor
description: Interactuar con el servidor aboocode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `aboocode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente aboocode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `aboocode.local` |
...
Configure `ABOOCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `aboocode`, o configure `ABOOCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `aboocode serve` como a `aboocode web`.
...
Cuando ejecuta `aboocode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor aboocode para interactuar con aboocode mediante programación.
:::

Esta arquitectura permite que aboocode admita múltiples clientes y le permite interactuar con aboocode mediante programación.

Puede ejecutar `aboocode serve` para iniciar un servidor independiente. Si tienes el
aboocode TUI ejecutándose, `aboocode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Aboocode [IDE](/docs/ide).
...
El servidor aboocode expone las siguientes API.
