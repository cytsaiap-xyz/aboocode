...
Puede agregar herramientas externas a Aboocode utilizando el _Model Context Protocol_ o MCP. Aboocode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Aboocode Config](https://opencode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/aboocode`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Aboocode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Aboocode:
...
Si el servidor requiere autenticación, Aboocode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `aboocode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Aboocode almacenará los tokens de forma segura en `~/.local/share/aboocode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Aboocode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Aboocode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Aboocode a su cuenta Sentry.
