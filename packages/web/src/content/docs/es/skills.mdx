...
Las habilidades del agente permiten a Aboocode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Aboocode busca estas ubicaciones:

- Configuración del proyecto: `.aboocode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/aboocode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Aboocode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.aboocode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/aboocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.aboocode/skills/git-release/SKILL.md` así:
...
Aboocode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `aboocode.json`:
...
**Para agentes integrados** (en `aboocode.json`):
