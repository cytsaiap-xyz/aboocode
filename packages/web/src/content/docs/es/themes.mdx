...
Con Aboocode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Aboocode usa nuestro propio tema `aboocode`.
...
Aboocode viene con varios temas integrados.
...
- Quiere que Aboocode coincida con la apariencia de su terminal
...
Aboocode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/aboocode/themes/*.json` o `$XDG_CONFIG_HOME/aboocode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.aboocode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.aboocode/themes/*.json`
