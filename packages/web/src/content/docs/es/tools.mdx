...
Las herramientas permiten que LLM realice acciones en su código base. Aboocode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Aboocode.
...
:::note
Esta herramienta solo está disponible cuando `ABOOCODE_EXPERIMENTAL_LSP_TOOL=true` (o `ABOOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Aboocode o cuando la variable de entorno `ABOOCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Aboocode:

```bash
ABOOCODE_ENABLE_EXA=1 aboocode
```
