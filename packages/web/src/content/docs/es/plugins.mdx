---
title: Complementos
description: Escriba sus propios complementos para extender Aboocode.
---

Los complementos le permiten extender Aboocode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Aboocode.
...
- `.aboocode/plugins/` - Complementos a nivel de proyecto
- `~/.config/aboocode/plugins/` - Complementos globales
...
```json title="aboocode.json"
{
  ...
  "plugin": ["aboocode-helicone-session", "aboocode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/aboocode/node_modules/`.
...
1. Configuración global (`~/.config/aboocode/aboocode.json`)
2. Configuración del proyecto (`aboocode.json`)
3. Directorio global de complementos (`~/.config/aboocode/plugins/`)
4. Directorio de complementos del proyecto (`.aboocode/plugins/`)
...
```json title=".aboocode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Aboocode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".aboocode/plugins/my-plugin.ts"
...
```js title=".aboocode/plugins/example.js"
...
- `client`: Un cliente SDK aboocode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar aboocode.
...
```js title=".aboocode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "aboocode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Aboocode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que aboocode lea archivos `.env`:

```javascript title=".aboocode/plugins/env-protection.js"
...
```javascript title=".aboocode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a aboocode:

```ts title=".aboocode/plugins/custom-tools.ts"
...
El ayudante `tool` crea una herramienta personalizada a la que aboocode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para aboocode junto con las herramientas integradas.
...
```ts title=".aboocode/plugins/compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
```ts title=".aboocode/plugins/custom-compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
