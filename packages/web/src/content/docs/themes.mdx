...
With Aboocode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Aboocode uses our own `aboocode` theme.
...
Aboocode comes with several built-in themes.
...
Aboocode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/aboocode/themes/*.json` or `$XDG_CONFIG_HOME/aboocode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.aboocode/themes/*.json`
4. **Current working directory** - Defined in `./.aboocode/themes/*.json`
