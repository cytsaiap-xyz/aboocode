...
U Aboocode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Aboocode koristi nasu `aboocode` temu.
...
Aboocode dolazi sa vise ugradenih tema.
...
- Zele da Aboocode odgovara izgledu njihovog terminala
...
Aboocode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/aboocode/themes/*.json` ili `$XDG_CONFIG_HOME/aboocode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.aboocode/themes/*.json`
4. **Trenutni radni direktorij** - `./.aboocode/themes/*.json`
