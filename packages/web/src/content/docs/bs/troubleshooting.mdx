...
Da biste otklonili probleme s Aboocode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\aboocode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `aboocode --log-level DEBUG`.

---

## Pohrana

Aboocode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/aboocode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\aboocode`
...
Aboocode Desktop pokreće lokalni Aboocode server (`aboocode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Aboocode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/aboocode/aboocode.jsonc` (ili `~/.config/aboocode/aboocode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/aboocode/aboocode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\aboocode\aboocode.jsonc`
...
Aboocode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/aboocode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\aboocode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Aboocode mogao ponovo izgraditi.

1. Potpuno zatvorite Aboocode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/aboocode`
- **Linux**: obrišite `~/.cache/aboocode` (ili pokrenite `rm -rf ~/.cache/aboocode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\aboocode`

3. Ponovo pokrenite Aboocode Desktop.

---

### Rješavanje problema sa vezom na serveru

Aboocode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `aboocode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `ABOOCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `ABOOCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Aboocode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Aboocode karakteristikama.

---

### Obavještenja se ne prikazuju

Aboocode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Aboocode u postavkama vašeg OS-a, i
...
1. Zatvorite Aboocode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Aboocode Desktop aplikacije):

- `aboocode.settings.dat` (URL zadanog servera za desktop)
- `aboocode.global.dat` i `aboocode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `aboocode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\aboocode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Aboocode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\aboocode`

2. Ponovo pokrenite Aboocode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Aboocode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Aboocode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
