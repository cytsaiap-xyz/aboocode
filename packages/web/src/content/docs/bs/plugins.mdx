---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Aboocode.
---

Dodaci vam omogućavaju da proširite Aboocode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Aboocode.
...
- `.aboocode/plugins/` - Dodaci na nivou projekta
- `~/.config/aboocode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/aboocode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/aboocode/aboocode.json`)
2. Konfiguracija projekta (`aboocode.json`)
3. Globalni direktorij dodataka (`~/.config/aboocode/plugins/`)
4. Direktorij dodataka projekta (`.aboocode/plugins/`)
...
Aboocode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Aboocode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Aboocode.
...
:::note
Ako koristite desktop aplikaciju Aboocode, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite aboocode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u aboocode:
...
Pomoćnik `tool` kreira prilagođeni alat koji aboocode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za aboocode zajedno sa ugrađenim alatima.
