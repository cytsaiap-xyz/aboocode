...
Agent skills omogucavaju Aboocode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Aboocode pretrazuje ove lokacije:

- Konfiguracija projekta: `.aboocode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/aboocode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Aboocode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.aboocode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/aboocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.aboocode/skills/git-release/SKILL.md` ovako:
...
Aboocode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `aboocode.json`:
...
**Za ugradene agente** (u `aboocode.json`):
