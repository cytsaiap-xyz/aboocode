...
Postoje dvije vrste agenata u Aboocode: primarni agenti i podagenti.
...
Aboocode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Aboocode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Aboocode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `aboocode.json`:
...
- Globalno: `~/.config/aboocode/agents/`
- Po projektu: `.aboocode/agents/`
...
Ako temperatura nije navedena, Aboocode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Aboocode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Aboocode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Aboocode Zen](/docs/zen), koristili biste `aboocode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `aboocode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/cytsaiap-xyz/aboocode).
:::
