---
title: Server
description: Komunicirajte s aboocode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `aboocode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji aboocode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `aboocode.local` |
...
Postavite `ABOOCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `aboocode`, ili postavite `ABOOCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `aboocode serve` i za `aboocode web`.
...
Kada pokrenete `aboocode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite aboocode server za programsku interakciju sa aboocode-om.
:::

Ova arhitektura omogucava aboocode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `aboocode serve` da startate standalone server. Ako je aboocode TUI vec pokrenut, `aboocode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Aboocode [IDE](/docs/ide) pluginovi.
...
aboocode server izlaže sljedece API-je.
