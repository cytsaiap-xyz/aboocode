---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Aboocode.
---

import config from "../../../../config.mjs"
export const console = config.console

Aboocode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Aboocode konfiguraciji.
...
u `~/.local/share/aboocode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Aboocode
config.
...
Aboocode Zen je lista modela koje je obezbedio Aboocode tim koji su bili
testirano i potvrđeno da dobro radi sa Aboocode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Aboocode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite aboocode i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u Aboocode i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s Aboocode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete aboocode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `aboocode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Aboocode.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi aboocode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju aboocode konfiguraciju.
...
da pohrani token u aboocode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Aboocode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Aboocode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`aboocode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `aboocode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s aboocode:
...
Za korištenje Google Vertex AI s Aboocode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće aboocode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem aboocodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [aboocode-helicone-session](https://github.com/H2Shami/aboocode-helicone-session) da automatski prijavite svaki Aboocode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Aboocode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati aboocode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati aboocode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati aboocode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Aboocode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/aboocode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Aboocode:
...
3. Kopirajte API ključ za korištenje u Aboocode.
...
6. **Važno**: Prije upotrebe modela oblaka u Aboocode, morate lokalno povući informacije o modelu:
...
Aboocode Zen je lista testiranih i verifikovanih modela koju je obezbedio Aboocode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Aboocode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Aboocode Zen**.

```txt
   /connect
```

3. Unesite svoj Aboocode API ključ.
...
Također možete dodati dodatne modele putem vaše aboocode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše aboocode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Aboocodeom:
...
Također možete prilagoditi modele kroz svoju aboocode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s aboocode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `aboocode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Aboocode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `aboocode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Aboocode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj aboocode konfiguraciji.
