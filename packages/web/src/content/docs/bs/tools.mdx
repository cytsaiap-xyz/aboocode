...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Aboocode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Aboocode.
...
:::note
Ovaj alat je dostupan samo kada je `ABOOCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `ABOOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Aboocode provajdera ili kada je varijabla `ABOOCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Aboocode:
