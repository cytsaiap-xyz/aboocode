...
Možete dodati vanjske alate u Aboocode koristeći _Model Context Protocol_, ili MCP. Aboocode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Aboocode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/aboocode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Aboocode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Aboocode će:
...
Ako server zahtijeva autentifikaciju, Aboocode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `aboocode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Aboocode sigurno čuva tokene u `~/.local/share/aboocode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Aboocode, zajedno s ugrađenim alatima. Možete njima upravljati kroz Aboocode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Aboocode sa vašim Sentry nalogom.
