...
ove ili konfigurirajte svoje putem aboocode config.
...
aboocode dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.aboocode/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `aboocode.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/aboocode/modes/`
- Projekat: `.aboocode/modes/`
...
Ako temperatura nije navedena, aboocode koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.aboocode/modes/` za specifične načine rada ili `~/.config/aboocode/modes/` za globalne načine:
