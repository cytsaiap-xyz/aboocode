---
title: Konfiguracija
description: Korištenje Aboocode JSON konfiguracije.
---

Možete konfigurirati Aboocode koristeći JSON konfiguracijski fajl.

---

## Format

Aboocode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Na primjer, ako vaša globalna konfiguracija postavlja `theme: "aboocode"` i `autoupdate: true`, a vaša projektna konfiguracija postavlja `model: "anthropic/claude-sonnet-4-5"`, konačna konfiguracija će uključivati ​​sve tri postavke.
...
1. **Udaljena konfiguracija** (od `.well-known/aboocode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/aboocode/aboocode.json`) - korisničke postavke
3. **Prilagođena konfiguracija** (`ABOOCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`aboocode.json` u projektu) - postavke specifične za projekat
5. **`.aboocode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`ABOOCODE_CONFIG_CONTENT` env var) - runtime preinačenja
   ...
   :::note
   Direktoriji `.aboocode` i `~/.config/aboocode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
   :::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/aboocode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Aboocode konfiguraciju u `~/.config/aboocode/aboocode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su teme, provajderi ili veze tipki.
...
Dodajte `aboocode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
:::tip
Postavite specifičnu konfiguraciju projekta u korijen vašeg projekta.
:::
Kada se Aboocode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `ABOOCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `ABOOCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.aboocode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.aboocode`, tako da **može nadjačati** njihove postavke.
...
Možete konfigurirati postavke servera za naredbe `aboocode serve` i `aboocode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Aboocode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `aboocode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Aboocode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Aboocode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Možete konfigurirati temu koju želite koristiti u svojoj Aboocode konfiguraciji putem opcije `theme`.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/aboocode/agents/` ili `.aboocode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Aboocode će se vratiti na `"build"` s upozorenjem.
Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`aboocode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/aboocode/commands/` ili `.aboocode/commands/`. [Saznajte više ovdje](/docs/commands).
...
Aboocode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Aboocode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Aboocode sa prilagođenim alatima, kukicama i integracijama.
Postavite datoteke dodataka u `.aboocode/plugins/` ili `~/.config/aboocode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Aboocode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
:::note
`disabled_providers` ima prioritet nad `enabled_providers`.
:::
