...
const managedConfigDir = process.env.ABOOCODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "aboocode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "aboocode.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "aboocode.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "aboocode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .aboocode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".aboocode")
...
test("loads agents from .aboocode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".aboocode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .aboocode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".aboocode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .aboocode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".aboocode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only ABOOCODE_CONFIG_DIR", async () => {
  ...
  const prev = process.env.ABOOCODE_CONFIG_DIR
  process.env.ABOOCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ABOOCODE_CONFIG_DIR
    else process.env.ABOOCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable ABOOCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "aboocode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .aboocode config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".aboocode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .aboocode config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "aboocode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .aboocode config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "aboocode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets ABOOCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .aboocode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .aboocode directory config enables it
      const opencodeDir = path.join(dir, ".aboocode")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "aboocode.json"),
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.aboocode/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.aboocode/plugin/oh-my-opencode.js")
  })
  ...
  test("local plugin directory overrides global aboocode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const opencodeDir = path.join(projectDir, ".aboocode")
...
describe("ABOOCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"]
    process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .aboocode/ directories when flag is set", async () => {
    const originalEnv = process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"]
    process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .aboocode directory with a command
          const opencodeDir = path.join(dir, ".aboocode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .aboocode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ABOOCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["ABOOCODE_CONFIG_DIR"]
      process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["ABOOCODE_CONFIG_DIR"]
      } else {
        process.env["ABOOCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("ABOOCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ABOOCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["ABOOCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["ABOOCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from ABOOCODE_CONFIG_DIR, not project
...
describe("ABOOCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in ABOOCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ABOOCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["ABOOCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["ABOOCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["ABOOCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in ABOOCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ABOOCODE_CONFIG_CONTENT"]
