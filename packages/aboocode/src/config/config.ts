...
export namespace Config {
  ...
  function getManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/aboocode"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "aboocode")
      default:
        return "/etc/aboocode"
    }
  }

  const managedConfigDir = process.env.ABOOCODE_TEST_MANAGED_CONFIG_DIR || getManagedConfigDir()
  ...
  async function initState() {
    ...
    // 2) Global config (~/.config/aboocode/aboocode.json{,c})
    // 3) Custom config (ABOOCODE_CONFIG)
    // 4) Project config (aboocode.json{,c})
    // 5) .aboocode directories (.aboocode/agents/, .aboocode/commands/, .aboocode/plugins/, .aboocode/aboocode.json{,c})
    // 6) Inline config (ABOOCODE_CONFIG_CONTENT)
    // Managed config directory is enterprise-only and always overrides everything above.
    let result: Info = {}
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        process.env[value.key] = value.token
        log.debug("fetching remote config", { url: `${key}/.well-known/aboocode` })
        const response = await fetch(`${key}/.well-known/aboocode`)
        if (!response.ok) {
        ...
        result = merge(
          ...
          await load(JSON.stringify(remoteConfig), {
            dir: path.dirname(`${key}/.well-known/aboocode`),
            source: `${key}/.well-known/aboocode`,
          }),
        )
    ...
    if (Flag.ABOOCODE_CONFIG) {
      result = merge(result, await loadFile(Flag.ABOOCODE_CONFIG))
      log.debug("loaded custom config", { path: Flag.ABOOCODE_CONFIG })
    }

    // Project config overrides global and remote config.
    if (!Flag.ABOOCODE_DISABLE_PROJECT_CONFIG) {
      for (const file of ["aboocode.jsonc", "aboocode.json"]) {
    ...
    if (Flag.ABOOCODE_CONFIG_DIR) {
      directories.push(Flag.ABOOCODE_CONFIG_DIR)
      log.debug("loading config from ABOOCODE_CONFIG_DIR", { path: Flag.ABOOCODE_CONFIG_DIR })
    }

    const deps = []

    for (const dir of unique(directories)) {
      if (dir.endsWith(".aboocode") || dir === Flag.ABOOCODE_CONFIG_DIR) {
        for (const file of ["aboocode.jsonc", "aboocode.json"]) {
    ...
    if (process.env.ABOOCODE_CONFIG_CONTENT) {
      result = merge(
        result,
        await load(process.env.ABOOCODE_CONFIG_CONTENT, {
          dir: Instance.directory,
          source: "ABOOCODE_CONFIG_CONTENT",
        }),
      )
      log.debug("loaded custom config from ABOOCODE_CONFIG_CONTENT")
    }
    ...
    if (Flag.ABOOCODE_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ABOOCODE_PERMISSION))
    }
    ...
    if (Flag.ABOOCODE_DISABLE_AUTOCOMPACT) {
      result.compaction = { ...result.compaction, auto: false }
    }
    if (Flag.ABOOCODE_DISABLE_PRUNE) {
  ...
  async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@aboocode/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@aboocode/plugin", depVersion, dir)
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.aboocode/command/", "/.aboocode/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.aboocode/agent/", "/.aboocode/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
   ...
   * 2. Local aboocode.json
   * 3. Global plugin/ directory
   * 4. Global aboocode.json
   *
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: aboocode.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for aboocode serve and web commands"),
      command: z
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "aboocode.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "aboocode.jsonc"))),
    )
  ...
  function globalConfigFile() {
    const candidates = ["aboocode.jsonc", "aboocode.json", "config.json"].map((file) =>
