...
import { fn } from "@aboocode/util/fn"
  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id = await Filesystem.readText(path.join(dotgit, "aboocode"))
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.ABOOCODE_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            await Filesystem.write(path.join(dotgit, "aboocode"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.ABOOCODE_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
