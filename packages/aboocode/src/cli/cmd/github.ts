...
const AGENT_USERNAME = "aboocode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/aboocode.yml"
...
jobs:
  aboocode:
    if: |
      ...
      startsWith(github.event.comment.body, '/aboocode')
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/aboocode,/oc")
          .split(",")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("aboocode-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `aboocode/${type}-${hex}-${timestamp}`
        }
        return `aboocode/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        ...
        const shareUrl = shareId ? `[aboocode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the aboocode infrastructure after your response",
