...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run aboo",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for aboocode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found aboocode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`aboo import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting aboocode...")
        UI.println()

        // Launch aboocode TUI with session ID if available
        const { spawn } = await import("child_process")
        const abooArgs = sessionId ? ["-s", sessionId] : []
        const abooProcess = spawn("aboo", abooArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          abooProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`aboocode exited with code ${code}`))
          })
          abooProcess.on("error", reject)
        })
      },
    })
  },
