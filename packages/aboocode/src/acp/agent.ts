...
import type { AssistantMessage, Event, AboocodeClient, SessionMessageResponse, ToolPart } from "@aboocode/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: AboocodeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `aboo auth login` in the terminal",
        name: "Login with aboocode",
        id: "aboocode-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // AbooCode stores files internally as { type: "file", url, filename, mime }.
