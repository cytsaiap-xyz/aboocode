...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for aboocode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal aboocode sessions
...
```bash
ABOOCODE_ENABLE_QUESTION_TOOL=1 aboo acp
```
...
- **Terminal Integration**: Full terminal support via aboocode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | aboo acp
```
...
ACP sessions map cleanly to aboocode's internal session model:
