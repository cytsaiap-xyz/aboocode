...
Replace runtime Bun shell template-tag usage in `packages/aboocode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/aboocode/src` is removed except:
