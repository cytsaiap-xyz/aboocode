...
const reg = process.env.REGISTRY ?? "ghcr.io/cytsaiap-xyz"
...
const setup = async () => {
  ...
  if (list.includes("aboocode")) {
    await $`docker buildx use aboocode`
    return
  }
  await $`docker buildx create --name aboocode --use`
}
