...
export const popularProviders = ["aboocode", "anthropic", "github-copilot", "openai", "google", "openrouter", "vercel"]
...
export function useProviders() {
  ...
  const paid = createMemo(() =>
    connected().filter((p) => p.id !== "aboocode" || Object.values(p.models).find((m) => m.cost?.input)),
  )
