...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("aboocode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("aboocode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("aboocode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("aboocode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("aboocode://open-project")).toBeUndefined()
    expect(parseDeepLink("aboocode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "aboocode://open-project?directory=/a",
      "aboocode://other?directory=/b",
      "aboocode://open-project?directory=/c",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __ABOOCODE__: {
        deepLinks: ["aboocode://open-project?directory=/a"],
      },
    } as unknown as Window & { __ABOOCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["aboocode://open-project?directory=/a"])
