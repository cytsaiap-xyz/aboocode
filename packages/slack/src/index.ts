...
import { createAboocode, type ToolPart } from "@aboocode/sdk"
...
console.log("🚀 Starting aboocode server...")
const aboocode = await createAboocode({
  port: 0,
})
console.log("✅ Aboocode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await aboocode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new aboocode session...")
    const { client, server } = aboocode
    ...
    console.log("✅ Created aboocode session:", createResult.data.id)
  ...
  console.log("📝 Sending to aboocode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Aboocode response:", JSON.stringify(result, null, 2))
