...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("aboocode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("aboocode_lib=debug,aboocode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("aboocode_lib=info,aboocode_desktop=info,sidecar=info")
        }
    });
