...
const CLI_INSTALL_DIR: &str = ".aboocode/bin";
const CLI_BINARY_NAME: &str = "aboocode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("aboocode-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "ABOOCODE_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "ABOOCODE_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("ABOOCODE_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.aboocode/bin/aboocode\"".to_string(),
            ...
            let mut env_prefix = vec![
                "ABOOCODE_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "ABOOCODE_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "ABOOCODE_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "ABOOCODE_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "ABOOCODE_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "ABOOCODE_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("ABOOCODE_SERVER_USERNAME", "aboocode".to_string()),
        ("ABOOCODE_SERVER_PASSWORD", password.to_string()),
    ];
    ...
    .expect("Failed to spawn aboocode");
