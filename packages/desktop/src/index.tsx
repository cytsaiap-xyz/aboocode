...
import { Splash } from "@aboocode/ui/logo"
...
const deepLinkEvent = "aboocode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__ABOOCODE__ ??= {}
  const pending = window.__ABOOCODE__.deepLinks ?? []
  window.__ABOOCODE__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__ABOOCODE__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__ABOOCODE__?.wsl) return result
  ...
  return {
    ...
    getWslEnabled: async () => {
      ...
      return window.__ABOOCODE__!.wsl ?? false
    },
