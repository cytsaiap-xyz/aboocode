...
const FORK_REPO = "cytsaiap-xyz/zed-extensions"
const UPSTREAM_REPO = "zed-industries/extensions"
const EXTENSION_NAME = "aboocode"
...
async function main() {
  ...
  // Delete any existing branches for aboocode updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating AbooCode extension to v${cleanVersion}"`
